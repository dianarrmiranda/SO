...
/*
  As funções pthread_cond_wait() e pthread_cond_timedwait() são usadas para bloquear uma variável de condição. 
  Eles são chamados com mutex bloqueado pelo thread de chamada ou um comportamento indefinido resultará.
  A função pthread_cond_wait() coloca a thread em espera (como se estivesse a dormir) até que a variável de condição seja sinalizada.
  A função pthread_cond_signal() sinaliza uma variável de condição, acordando uma thread que esteja à espera dessa variável de condição.

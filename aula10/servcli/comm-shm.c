...
void servOpenComm(void)
{
   bool first = false;
    ...
    if (first) { 
        ...
        up(R); //O servidor está pronto para receber pedidos 
    }
}
...
void servReceive(MESSAGE *msg)
{
   ...
   down(S); //O servidor vai ficar bloqueado até receber um pedido 
...
void servSend(MESSAGE *msg)
{
   ...
   *buf = *msg;
   ...
   /* the answer is ready */
   up(A); //O servidor está pronto para enviar a resposta 
}
...
void cliSend(MESSAGE *msg)
{
   ...
   down(R); //O cliente vai ficar bloqueado até o servidor estar pronto para receber pedidos 

   /* put message */
   *buf = *msg;

   /* notify server */
   up(S); //Notifica o servidor que o cliente está pronto para enviar um pedido
}
...
void cliReceive(MESSAGE *msg)
{
   ...
   down(A); //O cliente vai ficar bloqueado até o servidor estar pronto para enviar uma resposta

   /* get message */
   *msg = *buf;

   /* open access for a new request */
   up(R); //O cliente está pronto para enviar um novo pedido
}
